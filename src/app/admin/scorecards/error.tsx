...
export default function ScorecardsError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error("Scorecards route error caught:", error);
  }, [error]);
      ...
      <div className="max-w-md w-full rounded-2xl border border-red-200 dark:border-red-900/50 bg-white dark:bg-slate-900 p-6 text-center space-y-4 shadow-xl">
        ...
        <div className="space-y-1.5">
          <h2 className="text-lg font-black text-slate-900 dark:text-white">
            Scorecard Processing Error
          </h2>
          <p className="text-xs text-slate-600 dark:text-slate-400">
            {error?.message || "An issue occurred during scorecard intake or review."}
          </p>
        </div>
